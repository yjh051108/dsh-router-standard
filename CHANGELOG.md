...
## 0.2.3（2026-08-17，吸收 #29）

在 0.2.2 基础上吸收上游 PR #29（xxie-xd）——**请求剥离 AGENTS.md/技能目录注入**：

- `agent/pre-step` 过滤器（`prepend: true` 保证最外层）：会话晋升（首个持久 tool/call）前，
  从请求窗口剥离 `source.kind ∈ {agent-instructions, skill-catalog}` 的自动注入消息
- 消息仍持久化——晋升后自动回流到后续所有窗口，恢复无需任何代码
- 注入插件保持挂载（其工具属于晋升目录）；用户主动技能手势不在剥离集合
- 降级语义：下游错误（next）传播；仅过滤器自身逻辑错误降级为全保留
- 修复「干净 RL 首请求」前提（实测：V4 Flash we 轨迹锚 0/9 带目录 vs ~81% 无目录）

测试：26/26（新增 3 项：剥离/晋升回流+reject 直通/错误语义区分）。
